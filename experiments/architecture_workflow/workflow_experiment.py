    ...
    try:
        ...
        def _patched(self, *args, **kwargs):
            ...
            if hasattr(resp, "usage") and resp.usage:
                TOKEN_COLLECTOR.add(resp.usage.prompt_tokens, resp.usage.completion_tokens)
            return resp
...
class RunResult:
    ...
    def node_failure_rate(self) -> float:
        return sum(1 for n in self.nodes if not n.success) / len(self.nodes) if self.nodes else 0.0
...
def run_sequential(query: str) -> RunResult:
    """Sequential: market_context → analyst → sentiment → auditor → reporter."""
    return _run_workflow("sequential", create_sequential_graph,
...
def run_conditional(query: str) -> RunResult:
    ...
    return _run_workflow("conditional", create_conditional_graph,
...
def run_orchestration_worker(query: str) -> RunResult:
    ...
    return _run_workflow("orchestration_worker", create_orchestrator_graph,
...
DEFAULT_QUERIES: List[str] = [
    "Analyze AAPL and MSFT for long-term investment potential.",
...
def run_experiment(
    ...
    runners = {k: v for k, v in WORKFLOW_RUNNERS.items()
    ...
    for query in queries:
        ...
        for wf_name, runner in runners.items():
            ...
            result.judge_scores = llm_judge(query, result.final_report,
...
def print_comparison_table(results: List[RunResult]) -> None:
    ...
    cols = [
        ("Workflow",    "<", 25), ("N",        ">",  3), ("Lat(ms)",   ">", 10),
        ("Tokens",      ">",  8), ("In/node",  ">",  8), ("Out/node",  ">",  9),
        ("FailRate",    ">",  9), ("ToolErr",  ">",  8), ("Correct",   ">",  8),
        ("Relev",       ">",  6), ("Ground",   ">",  7), ("Guide",     ">",  6),
        ("Safety",      ">",  7),
    ]
    ...
    for r in results:
        ...
        print(f"  {r.workflow:<25}  {r.query[:48]:<50}  "
...
def save_results(results: List[RunResult], path: Optional[str] = None) -> str:
    """Serialise all RunResults to JSON; return the path written."""
    path = path or str(PROJECT_ROOT / "experiments" / "architecture_workflow" / "results.json")
    data = [
        {
            "workflow": r.workflow, "query": r.query,
            ...
            "nodes": [
                {"name": n.name, "latency_ms": round(n.latency_ms, 2),
                 "input_tokens": n.input_tokens, "output_tokens": n.output_tokens,
                 "tool_calls": n.tool_calls, "tool_errors": n.tool_errors,
                 "success": n.success, "error": n.error}
                for n in r.nodes
            ],
        }
        for r in results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--workflows", nargs="*", choices=list(WORKFLOW_RUNNERS),
    ...
    results = run_experiment(queries=args.queries, workflows=args.workflows)
