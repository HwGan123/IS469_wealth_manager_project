...
---------------------
  ...
  sentiment_agent            ← FinBERT sentiment analysis on news headlines
    │
    ▼
  investment_analyst_agent   ← writes draft investment report (RAG + market data + sentiment)
    │
    ▼
...
* Sentiment runs BEFORE the analyst so that the analyst can incorporate the
  bullish/bearish sentiment signal into the investment report.
