...
try:
    import anthropic
except ImportError:  # Optional dependency for live MCP tool calling
    anthropic = None
...
def market_context_node(state: WealthManagerState) -> dict:
    ...
    if not api_key or anthropic is None:
        logger.warning(
            "Anthropic client or API key not available. Falling back to empty market context."
        )
        return {
            "market_context": {
                "summary": "Live market context unavailable; using empty cached context.",
            },
            "audit_iteration_count": state.get("audit_iteration_count", 0),
        }
